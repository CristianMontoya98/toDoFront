...
function Card(props) {
  ...
  const handleEditDescription = ({ target: { value } }) =>
 ...
 of deleted in true, then actualize the list of tasks */
  ...
  const onDeleteDefinitive = (id) => {
    axios
      .delete(`https://fierce-castle-95757.herokuapp.com/api/todo/${id}`)
      .then(() => {
        setListTasks(
          listTasks.filter((val) => {
            return val.id !== id && val;
    ...
    >
      <div className={styles.cardTop}>
        ...
        <div className={styles.cardButton}>
          <button
            className={`${completed ? styles.btnCompleted : styles.btnCheck}`}
            onClick={() =>
              deleted ? onRestoreTodo(id) : onCheckTodo(id, title, description)
            }
          >
            {deleted ? (
          ...
          <button
            ...
            onClick={() =>
              deleted ? onDeleteDefinitive(id) : onDeleteTodo(id)
            }
          >
